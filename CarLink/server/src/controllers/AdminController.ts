...
export const GetAllUsers = async (
  ...
  try {
    ...
    const filteredCustomers = nonAdminCustomers.filter(
      (item) => item.role !== "admin"
    );

    const listCustomers = await Promise.all(
...
export const DeleteUser = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json("Người dùng và những thông tin liên quan đã được xoá thành công!");
  } catch (error) {
...
export const GetCarSAvailability = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const sendEmailServiceAccepted = async (email: string) => {
  ...
  const profileCar = await Car.findOne({
    where: { customerID: profileOwner?.customerID },
  });
  const profileOverview = await Overview.findOne({
    where: { carID: profileCar?.carID },
  });
...
//SEND EMAIL TO OWNER
export const sendEmailServiceDeclined = async (
  email: string,
  reason: string
) => {
  const profileOwner = await Customer.findOne({ where: { email: email } });
...
//USER SERVICE EMAIL
export const sendEmailServiceAcceptedBookingUser = async (
  email: string,
  bookingID: number
) => {
  const profileBooking = await Booking.findByPk(bookingID);
  ...
  const profileOverview = await Overview.findOne({
    where: { carID: profileBooking?.carID },
  });
...
export const MailAcceptBookingOwner = async (
  email: string,
  bookingID: number
) => {
  try {
...
export const MailAcceptBookingUser = async (
  email: string,
  bookingID: number
) => {
  try {
...
// GET ALL REPORTS with condition on bookingStatus in Booking
export const GetAllCompleteReports = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
//CONFIRM COMPLETE
export const ConfirmComplete = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if (pendingWithdrawals.length === 0)
      return res.status(404).json({
        message: "Không có yêu cầu rút tiền nào trong trạng thái pending.",
      });
...
export const GetApprovedOrCompletedWithdrawals = async (
  ...
  try {
    ...
    if (approvedOrCompletedWithdrawals.length === 0)
      return res.status(404).json({
        message:
          "Không có yêu cầu rút tiền nào trong trạng thái approval hoặc completed.",
      });
...
//SEND MAIL TO OWNER
export const sendEmailServiceAcceptedRequestWithdraw = async (
  email: string,
  withdrawID: number
) => {
  const profileOwner = await Customer.findOne({ where: { email: email } });
  ...
  const info = await transporter.sendMail({
    ...
    to: email, // list of receivers
    subject:
      "Thông báo: Yêu cầu rút tiền của bản trên CarLink đã được chấp thuận", // Subject line
    text: "Phản hồi dựa trên yêu cầu của bạn", // plain text body
    html: `<div>Kính gửi ${profileOwner?.firstName},
...
export const MailAcceptWithdrawOwner = async (
  email: string,
  withdrawID: number
) => {
  try {
...
export const ApproveWithdrawalRequest = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Đã chấp nhận yêu cầu rút tiền thành công!", withdraw });
  } catch (error) {
    res.status(500).json({
      message: "Đã xảy ra lỗi trong việc chấp nhận yêu cầu rút tiền!",
      error,
    });
  }
};
...
export const ConfirmWithdraw = async (req: Request, res: Response) => {
  try {
    ...
    const wallet = await Wallet.findOne({
      where: { customerID: withdraw?.customerID },
    });
    ...
    if (!withdraw || withdraw.status !== "approved") {
      return res
        .status(404)
        .json({ message: "Không tìm thấy yêu cầu rút tiền nào!" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Đã hoàn thành việc rút tiền!", withdraw });
  } catch (error) {
    res.status(500).json({
      message: "Đã xảy ra lỗi trong quá trình xác nhận rút tiền!",
      error,
    });
  }
};
